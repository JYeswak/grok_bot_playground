...
2. **Probe the plugin** — run the smallest read. Record the result: worked, error (quoted),
   or absent.
3. **Route** (this account 2026-09-12, *client-binary-verified*):
   - **Connector for that system worked** (Gmail, Calendar, Drive, X,
     reddit-mcp, linear-mcp, github-mcp, stripe-mcp, supabase) — use it.
     See `x-pulse`, `reddit-pulse`, `calendar-read`, `stripe-read`,
     `github-read`, `linear-aging`.
   - **Public page text** — `firecrawl-read`. Plugin is a **skill-pack (0 MCP)**. Working
     path is `firecrawl` CLI on the cloud computer (authenticated 2026-09-12, *site-verified*
     `example.com` scrape). Probe `firecrawl --status`; unauthenticated → HUMAN re-login
     on that screen. Empty `tool_census` is expected, not UNVERIFIED.
   - **Public page UI** — `browser-plugin` (`browser-use`, `playwright`,
     `browse`). Same-day census: names in MCP list, **zero tools**.
   - **Those plugins absent or erroring** — Agent Computer browser
     (`computer-drive`), read-only public content, label `BROWSER`.
   - **Neither can reach it** — refuse in one sentence, naming the
     human step (Settings → Plugins, attach to **this** Bot, OAuth).
   Do not open a browser for data a working connector already returned.
