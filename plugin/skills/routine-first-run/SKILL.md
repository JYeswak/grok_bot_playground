...
Limits, stated plainly: routines cannot be created over any API. A routine is
created by asking the Bot, in chat, to create it — the Bot can write its own
schedule when asked, and a human can edit it in the app. Anything claiming to
...
- Dependencies were proven before creation; a first run that failed on a
  ...
  needs another try".
- Nothing in the report claims a routine was created programmatically.
