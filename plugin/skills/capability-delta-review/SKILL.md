...
1. Newest audit `gates.enabled_ids` vs the `reviewed-audit:` baseline.
2. Id-diff first; names only describe a row the ids already found.
3. Whole returned object, not the field you came for.
...
Ids moved, not names. Every "no RPC" claim quotes the exact server answer and prefix. Adopt names an action and a verify; this skill never runs it.
...
- Report movement the vendor made, never movement your decoding made. A gained gate with no name is not a capability until a future bundle names it — record the id and the watch condition, nothing more.
- Never write "no read RPC exists" off one service prefix or one confused status: check every prefix and quote the exact server answer.
- This skill never enables, toggles, or adopts anything. "Adopt" names the single smallest action and how to verify it took effect; someone else runs it.
