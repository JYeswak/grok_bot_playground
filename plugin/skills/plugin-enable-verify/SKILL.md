...
3. **Make ONE read-only call per present plugin** — the pre-named probe, once.
   ...
   no second probe to "warm it up".
4. **Classify each result** into exactly one bucket:
   - **WORKING** — the probe returned data.
   - **AUTH** — reachable but rejected: the owner reopens the connector's detail
     page and re-authorizes with the intended account.
   - **ADMIN-BLOCKED** — the plugin reads **Disabled by team admin**. Stop on
     that plugin (see below).
   - **UNVERIFIED** — no probe was possible. Not a pass.
