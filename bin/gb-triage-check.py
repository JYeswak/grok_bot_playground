...
and 80 on the human line, because it renders up to 25 rows. That is the right
...
def selftest(root: pathlib.Path) -> int:
    """Prove the table: 25 ids, parity with the gate's CHECKS, fixes present, usage path."""
    ...
    try:
        ...
        return 1
    rows.append(("25 ids in CHECK_ORDER", len(CHECK_ORDER) == 25))
