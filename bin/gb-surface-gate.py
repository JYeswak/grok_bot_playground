...
def _scan_calls(
    ...
    for node in _ast.walk(tree):  # type: ignore[arg-type]
        if not isinstance(node, _ast.Call):
            continue
        func = node.func
        attr = isinstance(func, _ast.Attribute)
        name = func.attr if attr else getattr(func, "id", "")
        base = getattr(func.value, "id", "") if attr else ""
        ...
        elif attr and name in ("communicate", "wait") and not timed:
            unbounded.append(line)
...
def advisories(audit: dict | None) -> list[str]:
    ...
    if stale:
        out.append(
            f"{len(stale)} Bot(s) idle >{STALE_BOT_DAYS}d on a shared computer with live logins: "
...
def capabilities() -> dict:
    return {
        ...
        "selftest": "gb-surface-gate.py --selftest  (25 fixtures; mutation: --selftest --disable <check> must FAIL)",
    }
