...
from gbtypes import Cancelled, Cx, atomic_write_text, main  # noqa: E402
...
# Worst case ONE fetch can cost the region: the try, the one retry, the pause between them,
# and the per-host spacing. The region deadline in `build_document` is this times the budget,
# so a hung transport raises Cancelled instead of hanging the tick past what the budget
# promised. Named, not sprinkled: it moves if and only if the policy above moves.
FETCH_WORST_S: float = TIMEOUT * 2 + RETRY_SLEEP + PER_HOST_SLEEP
...
def harvest_doc(doc: Dict[str, Any], root: Root) -> List[Tuple[str, str]]:
...
def extract(body: bytes, content_type: str) -> Dict[str, Any]:
...
class _CountingRedirects(urllib.request.HTTPRedirectHandler):
    ...
    def redirect_request(
        ...
        """Count the hop, then follow it. `fp`/`headers` stay `Any` on purpose: typeshed
        leaves this handler untyped, and the selftest below drives it with `None` and a
        bare dict — the same loose values a real opener can hand it mid-redirect."""
        self.count += 1
...
def _request(url: str, timeout: int, max_bytes: int) -> Dict[str, Any]:
...
def fetch(
    ...
    *,
    ...
    retries: int = 1,
    request: Optional[Callable[[str, int, int], Dict[str, Any]]] = None,
...
class PoliteFetcher:
    ...
    def __init__(
        ...
        *,
        ...
        sleep: Callable[[float], None] = time.sleep,
        inner: Callable[[str], Dict[str, Any]] = fetch,
    ) -> None:
        self.gap = gap
        self.clock = clock
        self.sleep = sleep
...
def load_cache(root_dir: pathlib.Path) -> Tuple[Dict[str, Any], Optional[str]]:
...
def cache_age_days(entry: Dict[str, Any], now: dt.datetime) -> Optional[float]:
...
def cache_fresh(entry: Dict[str, Any], now: dt.datetime, max_age_days: float) -> bool:
...
def row_from_result(
    link: Link, result: Dict[str, Any], *, fetch_state: str
) -> Dict[str, Any]:
...
def deferred_row(link: Link) -> Dict[str, Any]:
...
def entry_from_result(result: Dict[str, Any]) -> Dict[str, Any]:
...
def result_from_entry(entry: Dict[str, Any]) -> Dict[str, Any]:
...
def plan(
    links: List[Link], cache: Dict[str, Any], *, now: dt.datetime, max_age_days: float
...
def build_document(
    *,
    ...
    cx = Cx(
        deadline=time.monotonic() + budget * FETCH_WORST_S,
        budget=budget,
    )
    for link in queue:
        if budget <= 0:
        ...
        # guard above, by construction. A cancelled run exits 130/143 with the previous
        # artifact intact, exactly like a SIGINT today.
...
def write_outputs(
    root_dir: pathlib.Path, doc: Dict[str, Any], cache: Dict[str, Any]
) -> pathlib.Path:
    ...
    for node in _ast.walk(_ast.parse(src)):
        ...
        for inner in _ast.walk(node):
            ...
            if isinstance(f, _ast.Attribute):
                name = f.attr
                base = f.value.id if isinstance(f.value, _ast.Name) else ""
...
def unguarded_prints(src: str) -> Set[str]:
...
def _fixture_links() -> Tuple[List[Link], List[Dict[str, Any]]]:
    merged: Dict[str, Link] = {}
    sources: List[Dict[str, Any]] = []
    pairs = (
...
def selftest() -> int:
    ...
    class _ScriptedTransport:
        ...
        A class, not a closure with a smuggled attribute: the call log is a declared field
        the checkers can see, so `flaky.calls` is typed instead of ignored."""

        def __init__(self, answers: Dict[str, List[Dict[str, Any]]]) -> None:
            self._answers = answers
            self.calls: List[str] = []

        def __call__(self, url: str, timeout: int, max_bytes: int) -> Dict[str, Any]:
            self.calls.append(url)
            queue = self._answers.get(url) or []
            base: Dict[str, Any] = {
                "url": url,
                "final_url": url,
                "status": None,
    ...
    def scripted(answers: Dict[str, List[Dict[str, Any]]]) -> _ScriptedTransport:
        """A fake transport: pops the next scripted answer for a url, recording every call."""
        return _ScriptedTransport(answers)
    ...
    # ---- region: an expired deadline aborts before the first request --------------------------
    # The region `build_document` builds comes from `time.monotonic`, so this leg pins the
    # clock to a fixed past instant: construction lands the deadline before any real instant,
    # while the checkpoint's own clock still reads the live one. Zero fetcher calls proves the
    # abort happened at the per-item boundary instead of after the spend; 143 is the SIGTERM
    # path through `main`, with the previous artifact intact.
    f_exp = FixtureFetcher()
    ...
    check(
        ...
        expired_code == 143 and not f_exp.calls,
        f"code={expired_code} calls={len(f_exp.calls)} — not the SIGTERM path, or spent first",
    )
    ...
    def strip(doc: Dict[str, Any]) -> str:
        out = []
