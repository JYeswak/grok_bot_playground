...
from gbtypes import Cancelled, Cx, atomic_write_json, main, read_json_capped  # noqa: E402
...
# Worst case ONE surface fetch can cost the region: the urlopen deadline plus the per-host
# politeness spacing. The region deadline in `collect` is this times the surface budget, so
# a hung transport raises Cancelled instead of hanging the tick past what the budget
# promised. Named, not sprinkled: it moves if and only if the policy above moves.
SURFACE_WORST_S: float = TIMEOUT_S + HOST_DELAY_S
...
def velocity_of(
    ...
    def in_days(limit: float) -> int:
        return sum(1 for a in ages if a <= limit)
...
def collect(
    ...
    *,
    ...
    # The region this run spends: one budget unit per fetch surface (the sitemap, the two
    # Cursor pages, x.ai/news). Built from the same timeout the fetcher enforces, so a healthy
    # run never trips it — every checkpoint below sees a positive budget and a live deadline
    # and is a no-op. A cancelled run unwinds to gbtypes.main, which exits 130/143 with the
    # previous artifact intact, exactly like a SIGINT today.
    budget = 2 + len(SURFACES)
    cx = Cx(
        deadline=time.monotonic() + budget * SURFACE_WORST_S,
        budget=budget,
    )

    # --- 1. the velocity instrument -------------------------------------------------------
    # The spine probe: cancel/deadline unwind here; the budget leg never fires past the fixed
    # surface count, by construction.
...
def _t11_cancellation_propagates() -> List[str]:
    ...
    urllib.request.urlopen = cancelling_urlopen
    try:
...
def _t14_checkpoint_fires_on_expired_deadline() -> List[str]:
    """An expired region deadline must abort `collect` BEFORE the first fetch, with 143.

    The region `collect` builds comes from `time.monotonic`, so this leg pins the clock to a
    fixed past instant: construction then lands the deadline before any real instant, while
    the checkpoint's own clock still reads the live one. Zero fetcher calls proves the abort
    happened at the per-item boundary instead of after the spend. `Cancelled(15).exit_code`
    is 143 — the same path a SIGTERM takes through `gbtypes.main`, with the previous
    artifact intact.
    """
    bad = []
    fetch_calls: List[str] = []
    fixture = _fixture_fetcher()

    def counting(url: str) -> Fetched:
        fetch_calls.append(url)
        return fixture(url)

    def pinned_past() -> float:
        return -1e12

    real_monotonic = time.monotonic
    time.monotonic = pinned_past
    code: Optional[int] = None
    try:
        collect(counting, T0, min_locs=1)
        bad.append("an expired region deadline did not abort collect()")
    except Cancelled as exc:
        code = exc.exit_code
    finally:
        time.monotonic = real_monotonic
    if code != 143:
        bad.append(f"expired deadline exited {code}, not 143 — not the SIGTERM path")
    if fetch_calls:
        bad.append(
            f"aborted AFTER spend: {len(fetch_calls)} fetches ran before the abort"
        )
    return bad
...
SELFTESTS: Tuple[Tuple[str, Callable[[], List[str]]], ...] = (
    ...
    (
        "t14 an expired region deadline aborts before the first fetch (143)",
        _t14_checkpoint_fires_on_expired_deadline,
    ),
)
