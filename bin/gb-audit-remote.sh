...
REMOTE_TMPDIR="/tmp/gb-audit.$$.d"
REMOTE_TMP="$REMOTE_TMPDIR/gb-deployment-audit.py"
STAMP="$(date -u +%Y-%m-%dT%H%M)"
OUT="$ROOT/deployment/desktops/${STAMP}.${LABEL}.json"
mkdir -p "$(dirname "$OUT")"

# Ship the audit PLUS its house imports: gb-deployment-audit.py imports gblib and
# gbtypes at module scope (shared-reader extraction), and shipping the one file alone
# dies on the remote with ModuleNotFoundError. Same temp dir so its sys.path insert
# resolves; removed afterwards either way.
scp -q -o BatchMode=yes -o ConnectTimeout=10 "$ROOT/bin/gb-deployment-audit.py" "$ROOT/bin/gblib.py" "$ROOT/bin/gbtypes.py" "$ALIAS:$REMOTE_TMPDIR/" 2>/dev/null || {
  ssh -o BatchMode=yes -o ConnectTimeout=10 "$ALIAS" "mkdir -p '$REMOTE_TMPDIR'" &&
  scp -q -o BatchMode=yes -o ConnectTimeout=10 "$ROOT/bin/gb-deployment-audit.py" "$ROOT/bin/gblib.py" "$ROOT/bin/gbtypes.py" "$ALIAS:$REMOTE_TMPDIR/"
}
...
ssh -o BatchMode=yes -o ConnectTimeout=10 "$ALIAS" \
  "python3 '$REMOTE_TMP' --no-decode --out '$REMOTE_TMPDIR/out.json'; rc=\$?; cat '$REMOTE_TMPDIR/out.json' 2>/dev/null; rm -rf '$REMOTE_TMPDIR'; exit \$rc" \
  > "$OUT.raw"
rc=$?
