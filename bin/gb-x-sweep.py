...
TAXONOMY_RX: tuple[tuple[str, re.Pattern[str]], ...] = tuple(
    (name, re.compile(rx, re.I)) for name, rx in TAXONOMY
)


def categorise(text: Optional[str]) -> list[str]:
...
# -------------------------------------------------------------------------------------------
def rank_uses(use_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
...
def read_first_pass(path: Optional[pathlib.Path]) -> dict[str, Any]:
...
def collect(
    *,
    fetch: Optional[
        Callable[[str, int], tuple[Optional[dict[str, Any]], Optional[str], int]]
    ] = None,
    now: Optional[dt.datetime] = None,
    queries: tuple[dict[str, Any], ...] = QUERIES,
    ...
    """
    _fetch: Callable[
        [str, int], tuple[Optional[dict[str, Any]], Optional[str], int]
    ] = gbx.x_search if fetch is None else fetch
    ...
    for spec in queries:
        ...
        if calls and sleep_s:
            sleep(sleep_s)
        calls += 1
        envelope, error, status = _fetch(q, int(spec["max"]))
        row = {
            "id": label,
            "query": q,
            "role": spec["role"],
...
def refresh_use(row: dict[str, Any]) -> None:
...
def synthetic_rows(
    prior_uses: list[dict[str, Any]], exclude_rows: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    ...
    seen: set[Any] = {r["id"] for r in exclude_rows}
    out: list[dict[str, Any]] = []
    for u in prior_uses:
        if u["id"] in seen:
            continue
...
def reclassify(first: dict[str, Any], sweep: dict[str, Any]) -> dict[str, Any]:
    ...
    authored_authors: set[Any] = set()
    relayed_ids: set[Any] = set()
...
def query_yield(source_row: dict[str, Any]) -> dict[str, Any]:
...
def ranked_lines(ranked: list[dict[str, Any]], limit: int = 15) -> list[str]:
...
def report_reclassify(doc: dict[str, Any]) -> None:
    k = doc["combined"]
    emit(
...
FX_AUTHORED_QUOTE: dict[str, Any] = {
...
FX_AUTHORED_ROLE_COLON: dict[str, Any] = {
...
FX_AUTHORED_SUMMARY_OF: dict[str, Any] = {
...
FX_AUTHORED_NOTES_FROM: dict[str, Any] = {
...
FX_RELAY_ROLE_LEAD: dict[str, Any] = {
...
) -> "tuple[Callable[[str, int], tuple[Optional[dict[str, Any]], Optional[str], int]], list[tuple[str, int]]]":
...
CTRL: dict[str, list[dict[str, Any]]] = {
    "grok -is:retweet": [FX_DEPLOYED],
    '("grok bot" OR grokbot) -has:links -is:retweet': [FX_NOT_A_USE],
}


def emit_verdict(checks: list[tuple[str, bool, str]], passed: int, total: int) -> None:
...
def selftest() -> int:
    ...
    check(
        ...
        good is not None
        and good["tier"] == "deployed"
        and "possessive" in good["patterns"],
        str(good),
    )
    ...
    check(
        ...
        and intended["tier"] == "intended"
        and intended["solicited"],
        str(intended),
    )
    ...
    check(
        ...
        all(
            v is not None and v["relay"] and v["relay_reason"] == want
    ...
    check(
        ...
        all(v is not None and v["relay"] is False for v in authored.values()),
    ...
    check(
        ...
        all(v is not None and v["is_use"] for v in authored.values()),
